    ...
    def __init__(self, nb_actions, scope):
        with tf.variable_scope(scope):
            ...
            if scope != 'target':
                ...
                gradients = optimizer.compute_gradients(self.action_value_loss, var_list=tf.trainable_variables())
                for i, (grad, var) in enumerate(gradients):
                    if grad is not None:
                        gradients[i] = (tf.clip_by_norm(grad, FLAGS.gradient_norm_clipping), var)
