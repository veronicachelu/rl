    ...
    def __init__(self, nb_actions, scope):
        with tf.variable_scope(scope):
            ...
            with tf.variable_scope("action_value"):
                ...
                value_out = tf.split(self.action_values, num_or_size_splits=nb_actions, axis=1)
                self.action_values_soft = tf.concat(list(map(lambda v: tf.nn.softmax(v), value_out)), 1)
